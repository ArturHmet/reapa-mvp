      ...
      <div className="bg-gradient-to-r from-indigo-600/20 to-purple-600/20 border border-indigo-500/20 rounded-xl p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <h1 className="text-xl font-bold mb-1">Good morning! Here&apos;s your day.</h1>
            <p className="text-sm text-[var(--text-muted)]">{today}</p>
          </div>
          <div className="text-right">
            <div className="text-2xl font-bold text-indigo-400">{dashboardStats.avgResponseTime}</div>
        ...
        <Card className="col-span-2">
          ...
          <div className="space-y-2">
            {tasks.filter(t => t.status !== 'done').slice(0, 7).map(task => (
              <div key={task.id} className="flex items-center gap-3 p-3 rounded-lg bg-[var(--bg)] hover:bg-[var(--bg-card-hover)] transition-colors group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-[var(--text-muted)] truncate">{task.description}</div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  {task.autoGenerated && (
                    <span className="text-[10px] text-indigo-400 bg-indigo-500/10 px-1.5 py-0.5 rounded">AI</span>
                  )}
                  <CategoryIcon category={task.category} />
                  <span className="text-[11px] text-[var(--text-muted)]">{task.dueTime}</span>
