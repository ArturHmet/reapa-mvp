...
export async function GET() {
  try {
    ...
    const [{ count: totalLeads }, { count: hotLeads }, { count: activeClients }, { count: pendingTasks }, { count: overdueTasks }] = await Promise.all([
    ...
    const { data: sourceData } = await admin.from("leads").select("source");
    ...
    const leadSourceData = Object.entries(sourceCounts).map(([source, count]) => ({
      source: source.charAt(0).toUpperCase() + source.slice(1), count,
      percentage: Math.round((count / total) * 100),
    }));
    const { count: qualifiedLeads } = await admin.from("leads").select("*", { count: "exact", head: true }).in("temperature", ["hot", "warm"]);
    const { count: closedLeads } = await admin.from("leads").select("*", { count: "exact", head: true }).eq("status", "closed_won");
    const funnelData = [
    ...
    return NextResponse.json({
      totalLeads: totalLeads || 0, hotLeads: hotLeads || 0, activeClients: activeClients || 0,
      viewingsToday: 1, pendingTasks: pendingTasks || 0, overdueTasks: overdueTasks || 0,
      dealsThisMonth: closedLeads || 0, revenue: (closedLeads || 0) * 14250,
