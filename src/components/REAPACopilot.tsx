...
import { X, Minimize2, Maximize2, MessageSquare, Send, Bot } from "lucide-react";
...
export function REAPACopilot() {
  ...
  // ── BUG FIX: Task 5 — use append() ONLY, not handleSubmit() + form.requestSubmit()
  // Old code (double-submit bug): handleSubmit(e) + form.requestSubmit() = 2 submissions
  // Fixed: append() sends message directly without form events
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 flex flex-col bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl transition-all ${
        isMinimized ? "w-72 h-14" : "w-96 h-[560px]"
      }`}
    >
      {/* Header */}
          ...
          <div>
            ...
            {!isMinimized && (
              <p className="text-xs text-gray-400">
                {isLoading ? "Thinking…" : "AI assistant · Always on"}
              </p>
            )}
          </div>
        </div>
